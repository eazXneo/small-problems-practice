...
# CURRENTLY ABANDONING STACK VERSION OF DFS.
# would need a "visited" array or something to keep track of nodes already
# acessed when backtracking.
def MY_has_path(root, sum):  # from Q I guess it returns boolean. # Q literally says 'S'
    ...
    while len(stack) != 0:
        ...
        if current_node not in visited:
            ...
            # trying to backtrack with the total sum
            current_sum -= current_node.val
    return False
# deque vs list rep...?
...
# right children
# improvement in this case, if current_sum > sum, then abort that path entirely, saving time?

## ANSWER
# drew out the recursion tree and I think it makes sense, ish.
...
def main():
    ...
    print("Tree has path: " + str(MY_has_path(root, 23)))
    print("Tree has path: " + str(MY_has_path(root, 16)))
